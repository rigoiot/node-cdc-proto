...
proto.CDCMsg.prototype.setData = function(data) {
  var buf;
 ...
 * @returns {Buffer}
 */
proto.CDCMsg.prototype.getRawData = function() {
  return new Buffer(this.getPayload(), 'binary');
};

/**
 * Convenience API to convert the results of CDCMsg#getRawData to
 * a string with the specified 'encoding'. Note that if the encoding
 * is set to binary, this method returns CDCMsg#getRawData.
 * @returns {!(string|Uint8Array)|string}
 */
proto.CDCMsg.prototype.getData = function(encoding) {
  var bytes = this.getPayload();
